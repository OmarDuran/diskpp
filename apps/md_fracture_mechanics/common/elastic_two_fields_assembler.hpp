...
struct spring {
    ...
    std::pair<size_t,size_t> m_p_index;
    std::pair<size_t,size_t> m_s_index;
    std::pair<size_t,size_t> m_l_index;
...
struct fracture {
    ...
    std::vector<std::pair<size_t,size_t>> m_internal_edges;
    ...
    fracture(const fracture &other){
        ...
        m_internal_edges = other.m_internal_edges;
    }
...
//        m_skin_bs = 1*(4 * m_pairs.size() + 1);
    ...
    void build_mesh(const Mesh& msh){
        ...
        for (auto chunk : m_pairs) {
            ...
            if(cell_c != m_pairs.size() - 1){
                m_internal_edges.push_back(std::make_pair(node_l, node_r));
            }
            cell_c++;
...
class elastic_two_fields_assembler
{
    ...
    // skin cells and edges indexes
    std::vector<size_t>                 m_compress_skin_cells_indexes;
    std::vector<size_t>                 m_compress_skin_edges_indexes;
    ...
    size_t      m_n_skin_dof;
    size_t      m_n_skin_cells_dof;
    size_t      m_n_skin_edges_dof;
    size_t      m_n_skin_bc_edges_dof;
    size_t      m_sigma_degree;
    size_t      m_n_up_mortar_dof;
    size_t      m_n_mortar_points;
    size_t      m_n_spring_points;
    size_t      m_n_spring_dof;
...
public:
    ...
    elastic_two_fields_assembler(const Mesh& msh, const disk::hho_degree_info& hho_di, const boundary_type& bnd, const std::vector<std::pair<size_t,size_t>> & fracture_pairs, std::vector<std::pair<size_t,size_t>> & end_point_mortars, std::vector<fracture<Mesh> > & fractures, std::vector<restriction > & restrictions, std::vector<spring > & springs)
        : m_hho_di(hho_di), m_bnd(bnd), m_fracture_pairs(fracture_pairs), m_end_point_mortars(end_point_mortars), m_fractures(fractures), m_restrictions(restrictions), m_springs(springs), m_hho_stabilization_Q(true), m_scaled_stabilization_Q(false)
    {
        ...
        m_compress_skin_cells_indexes.resize(m_fractures.size());
        m_compress_skin_edges_indexes.resize(m_fractures.size());
        m_n_skin_cells_dof = 0;
        m_n_skin_edges_dof = 0;
        m_n_skin_bc_edges_dof = 0;
        ...
        for (auto f : m_fractures) {
            ...
            m_compress_skin_cells_indexes.at(frac_c) = m_n_skin_cells_dof;
            m_compress_skin_edges_indexes.at(frac_c) = m_n_skin_edges_dof;
            ...
            m_n_skin_cells_dof +=  3 * 4 * f.m_pairs.size();
            m_n_skin_edges_dof +=  1 * 4 * (f.m_pairs.size()-1);
            
            if(f.m_bc_type.first == 1){
                m_n_skin_bc_edges_dof += 2;
            }
                
            if(f.m_bc_type.second == 1){
                m_n_skin_bc_edges_dof += 2;
            }
        ...
        m_n_mortar_points = m_restrictions.size();
        ...
        m_n_spring_points = m_springs.size();
        m_n_spring_dof = 0;
        for (size_t i = 0; i < m_n_spring_points; i++) {
            m_n_spring_dof += 2;
        }
        ...
        system_size += m_n_up_mortar_dof + m_n_skin_bc_edges_dof + m_n_spring_dof;
    ...
    void scatter_mortar_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mortar_mat)
    {
        ...
        frac_LHS_offset += m_n_up_mortar_dof + m_n_skin_bc_edges_dof;
    ...
    void scatter_skins_point_mortar_rhs_t_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const Matrix<T, Dynamic, Dynamic>& rhs)
    {
    ...
    void scatter_skins_bc_u_t_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, size_t bc_ind, size_t s_ind)
    {
        ...
        size_t n_bc_skin_bs = 4;
        std::vector<assembly_index> asm_map;
        auto base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof + m_n_up_mortar_dof;
        auto u_LHS_offset = base + bc_ind * n_bc_skin_bs;
        ...
        for (size_t i = 0; i < mat.rows(); i++)
        {
            ...
            {
                m_triplets.push_back( Triplet<T>(asm_map[i]+1, asm_map[j]+1, +1.0*mat(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map[i]+3, asm_map[j]+3, +1.0*mat(i,j)) );
            }
            RHS(asm_map[i]+1) += +1.0*uD_val * beta;
            RHS(asm_map[i]+3) += -1.0*uD_val * beta;
        }
        
    }
    
    void scatter_skins_point_restriction_u_n_data(const Mesh& msh, restriction & r, const Matrix<T, Dynamic, Dynamic>& mat)
    {
    ...
    void scatter_skins_point_restriction_u_t_data(const Mesh& msh, restriction & r, const Matrix<T, Dynamic, Dynamic>& mat)
    {
    ...
    void scatter_mortar_rhs_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mortar_rhs)
    {
        size_t n_f_sigma_bs = 2.0*disk::scalar_basis_size(m_sigma_degree, Mesh::dimension-1);
    ...
    void scatter_skin_weighted_mass_l_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mass_mat)
    {
        ...
        size_t n_sigma_skin_l_bs = n_sigma_skin_bs * f.m_pairs.size();
        std::vector<assembly_index> asm_map;
        auto skin_LHS_offset = m_n_cells_dof + m_n_faces_dof + cell_ind * n_sigma_skin_bs;
        skin_LHS_offset += m_compress_skin_cells_indexes.at(fracture_ind);
    ...
    void scatter_skin_weighted_ul_n_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_fbs = disk::vector_basis_size(m_hho_di.face_degree(), Mesh::dimension - 1, Mesh::dimension);
    ...
    void scatter_skin_weighted_ul_t_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_sigma_skin_l_bs = n_sigma_skin_bs * f.m_pairs.size();
        size_t n_fbs = disk::vector_basis_size(m_hho_di.face_degree(), Mesh::dimension - 1, Mesh::dimension);
    ...
    void scatter_skin_weighted_ur_n_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_sigma_skin_r_bs = n_sigma_skin_bs * f.m_pairs.size();
        size_t n_fbs = disk::vector_basis_size(m_hho_di.face_degree(), Mesh::dimension - 1, Mesh::dimension);
    ...
    void scatter_skin_weighted_ur_t_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_sigma_skin_l_bs = n_sigma_skin_bs * f.m_pairs.size();
        size_t n_fbs = disk::vector_basis_size(m_hho_di.face_degree(), Mesh::dimension - 1, Mesh::dimension);
    ...
    void scatter_skin_hybrid_l_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_cells = f.m_pairs.size();
        size_t n_skin_l_bs = n_sigma_skin_bs * n_cells;
        std::vector<assembly_index> asm_map_i, asm_map_l_j, asm_map_r_j;
        auto base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof;
    ...
    void scatter_skin_hybrid_r_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
    ...
    void scatter_skin_hybrid_l_nodewise_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & node_loc, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_cells = f.m_pairs.size();
        size_t n_nodes = f.m_internal_edges.size();
        size_t n_skin_l_bs = n_sigma_skin_bs * n_cells;
        std::vector<assembly_index> asm_map_m, asm_map_p, asm_map_e;
        auto skin_base = m_n_cells_dof + m_n_faces_dof + m_compress_skin_cells_indexes.at(fracture_ind);
        auto edge_base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_compress_skin_edges_indexes.at(fracture_ind);
        auto skin_LHS_offset_m = skin_base + (node_loc) * n_sigma_skin_bs;
        auto skin_LHS_offset_p = skin_base + (node_loc+1) * n_sigma_skin_bs;
        auto edge_LHS_offset = edge_base + node_loc;
        
        for (size_t i = 0; i < n_sigma_skin_bs; i++)
        asm_map_m.push_back( assembly_index(skin_LHS_offset_m+i, true));

        for (size_t j = 0; j < n_sigma_skin_bs; j++)
        asm_map_p.push_back( assembly_index(skin_LHS_offset_p+j, true));
        
        for (size_t j = 0; j < 1; j++)
        asm_map_e.push_back( assembly_index(edge_LHS_offset+j, true));
        
        Matrix<T, Dynamic, Dynamic> mat_m = mat.block(0,0,3,1);
        Matrix<T, Dynamic, Dynamic> mat_p = mat.block(0,1,3,1);
        assert( asm_map_m.size() == mat_m.rows() && asm_map_e.size() == mat_m.cols() );
        assert( asm_map_p.size() == mat_p.rows() && asm_map_e.size() == mat_p.cols() );

        for (size_t i = 0; i < mat_m.rows(); i++)
        {
            for (size_t j = 0; j < mat_m.cols(); j++)
            {
                m_triplets.push_back( Triplet<T>(asm_map_m[i], asm_map_e[j],+1.0*mat_m(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j], asm_map_m[i],+1.0*mat_m(i,j)) );

                m_triplets.push_back( Triplet<T>(asm_map_m[i]+n_skin_l_bs, asm_map_e[j]+n_nodes,+1.0*mat_m(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+n_nodes, asm_map_m[i]+n_skin_l_bs,+1.0*mat_m(i,j)) );
                
            }
        }
        
        for (size_t i = 0; i < mat_p.rows(); i++)
        {
            for (size_t j = 0; j < mat_p.cols(); j++)
            {
                m_triplets.push_back( Triplet<T>(asm_map_p[i], asm_map_e[j],+1.0*mat_p(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j], asm_map_p[i],+1.0*mat_p(i,j)) );

                m_triplets.push_back( Triplet<T>(asm_map_p[i]+n_skin_l_bs, asm_map_e[j]+n_nodes,+1.0*mat_p(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+n_nodes, asm_map_p[i]+n_skin_l_bs,+1.0*mat_p(i,j)) );
    ...
    void scatter_skin_hybrid_r_nodewise_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & node_loc, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_cells = f.m_pairs.size();
        size_t n_nodes = f.m_internal_edges.size();
        size_t n_skin_l_bs = n_sigma_skin_bs * n_cells;
        std::vector<assembly_index> asm_map_m, asm_map_p, asm_map_e;
        auto skin_base = m_n_cells_dof + m_n_faces_dof + 2.0 * n_skin_l_bs + m_compress_skin_cells_indexes.at(fracture_ind);
        auto edge_base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + 2.0 * n_nodes + m_compress_skin_edges_indexes.at(fracture_ind);
        auto skin_LHS_offset_m = skin_base + (node_loc) * n_sigma_skin_bs;
        auto skin_LHS_offset_p = skin_base + (node_loc+1) * n_sigma_skin_bs;
        auto edge_LHS_offset = edge_base + node_loc;
    ...
    void scatter_skin_hybrid_nodewise_data(const Mesh& msh, size_t f_ind_l, fracture<Mesh> & f_l, size_t & node_l, bool side_Q_l, size_t f_ind_r, fracture<Mesh> & f_r, const size_t & node_r, bool side_Q_r, size_t & restrict_node, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_nodes_bs = 2;
        ...
        std::vector<assembly_index> asm_map_l, asm_map_r, asm_map_e;
        auto skin_l_base = m_n_cells_dof + m_n_faces_dof + m_compress_skin_cells_indexes.at(f_ind_l);
        auto skin_r_base = m_n_cells_dof + m_n_faces_dof + m_compress_skin_cells_indexes.at(f_ind_r);
        auto edge_base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof;
        
        if(!side_Q_l){
            skin_l_base += 2 * n_skin_bs_l;
        }
        
        if(!side_Q_r){
            skin_r_base += 2 * n_skin_bs_r;
        }
        ...
        for (size_t j = 0; j < n_sigma_skin_bs; j++)
        asm_map_r.push_back( assembly_index(skin_LHS_offset_r+j, true));
        ...
        Matrix<T, Dynamic, Dynamic> mat_l = mat.block(0,0,3,1);
        Matrix<T, Dynamic, Dynamic> mat_r = mat.block(0,1,3,1);
        assert( asm_map_l.size() == mat_l.rows() && asm_map_e.size() == mat_l.cols() );
        assert( asm_map_r.size() == mat_r.rows() && asm_map_e.size() == mat_r.cols() );
        ...
        for (size_t i = 0; i < mat_r.rows(); i++)
        {
            for (size_t j = 0; j < mat_r.cols(); j++)
            {
    ...
    void scatter_skin_hybrid_spring_data(const Mesh& msh, size_t f_ind_l, fracture<Mesh> & f_l, size_t & node_ind_l, bool side_Q_l, size_t f_ind_r, fracture<Mesh> & f_r, const size_t & node_ind_r, bool side_Q_r, size_t & spring_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_nodes_bs = 2;
        size_t s_nodes = m_springs.size();
...
//        size_t n_skin_bs_r = n_sigma_skin_bs * n_cells_r;
        ...
        std::vector<assembly_index> asm_map, asm_map_e;
        auto edge_base_l = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof;
        auto edge_base_r = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof;
        auto spring_base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof;
        spring_base += m_n_up_mortar_dof + m_n_skin_bc_edges_dof + m_n_f_hybrid_dof;
        
        auto edge_LHS_offset_l = edge_base_l + node_ind_l*2+1;
        auto edge_LHS_offset_r = edge_base_r + node_ind_r*2+1;
        auto spring_LHS_offset = spring_base + spring_ind*2;
        
        for (size_t i = 0; i < 1; i++)
        asm_map.push_back( assembly_index(edge_LHS_offset_l+i, true));

        for (size_t j = 0; j < 1; j++)
        asm_map.push_back( assembly_index(edge_LHS_offset_r+j, true));
        
        for (size_t j = 0; j < 1; j++)
        asm_map_e.push_back( assembly_index(spring_LHS_offset+j, true));
    
        for (size_t i = 0; i < mat.rows(); i++)
        {
            for (size_t j = 0; j < mat.cols(); j++)
            {
                m_triplets.push_back( Triplet<T>(asm_map[i], asm_map_e[j]+1,+1.0*mat(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+1, asm_map[i],+1.0*mat(i,j)) );
                
//                m_triplets.push_back( Triplet<T>(asm_map[i], asm_map_e[j],+1.0*mat(i,j)) );
//                m_triplets.push_back( Triplet<T>(asm_map_e[j], asm_map[i],+1.0*mat(i,j)) );
                
            }
        }
        
        m_triplets.push_back( Triplet<T>(asm_map_e[0], asm_map_e[0],+1.0) );
        m_triplets.push_back( Triplet<T>(asm_map_e[0]+1, asm_map_e[0]+1,+1.0) );
    
    }
    
    void scatter_skin_hybrid_bc_data(const Mesh& msh, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const size_t & bc_ind, const Matrix<T, Dynamic, Dynamic>& mat)
    {
        size_t n_sigma_skin_bs = 3;
        size_t n_bc_skin_bs = 4;
        ...
        std::vector<assembly_index> asm_map_m, asm_map_p, asm_map_e;
        auto skin_base = m_n_cells_dof + m_n_faces_dof + m_compress_skin_cells_indexes.at(fracture_ind);
        auto edge_base = m_n_cells_dof + m_n_faces_dof + m_n_skin_cells_dof + m_n_skin_edges_dof + m_n_up_mortar_dof;
        auto skin_LHS_offset_l = skin_base + (cell_ind) * n_sigma_skin_bs;
        auto skin_LHS_offset_r = skin_base + (cell_ind) * n_sigma_skin_bs + 2.0 * n_skin_l_bs;
        auto edge_LHS_offset = edge_base + bc_ind * n_bc_skin_bs;
        
        for (size_t i = 0; i < n_sigma_skin_bs; i++)
        asm_map_m.push_back( assembly_index(skin_LHS_offset_l+i, true));

        for (size_t j = 0; j < n_sigma_skin_bs; j++)
        asm_map_p.push_back( assembly_index(skin_LHS_offset_r+j, true));
        ...
        for (size_t i = 0; i < mat_m.rows(); i++)
        {
            for (size_t j = 0; j < mat_m.cols(); j++)
            {
                ...
                m_triplets.push_back( Triplet<T>(asm_map_m[i]+n_skin_l_bs, asm_map_e[j]+1,+1.0*mat_m(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+1, asm_map_m[i]+n_skin_l_bs,+1.0*mat_m(i,j)) );
        ...
        for (size_t i = 0; i < mat_p.rows(); i++)
        {
            for (size_t j = 0; j < mat_p.cols(); j++)
            {
                m_triplets.push_back( Triplet<T>(asm_map_p[i], asm_map_e[j]+2,+1.0*mat_p(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+2, asm_map_p[i],+1.0*mat_p(i,j)) );

                m_triplets.push_back( Triplet<T>(asm_map_p[i]+n_skin_l_bs, asm_map_e[j]+3,+1.0*mat_p(i,j)) );
                m_triplets.push_back( Triplet<T>(asm_map_e[j]+3, asm_map_p[i]+n_skin_l_bs,+1.0*mat_p(i,j)) );
...
//                scatter_skin_weighted_ur_n_data(msh, chunk.second, f_ind, f, cell_ind, ur_div_phi.first);
                ...
                if(1){
                    ...
                    auto rhs_l = skin_coupling_rhs_u(msh, cell_l, face_l, f, cell_ind, rhs_l_fun);
                    scatter_skin_weighted_rhs_u_data(msh, chunk.first, f_ind, f, cell_ind, rhs_l.second);
                    
                    auto rhs_r = skin_coupling_rhs_u(msh, cell_r, face_r, f, cell_ind, rhs_r_fun);
                    scatter_skin_weighted_rhs_u_data(msh, chunk.second, f_ind, f, cell_ind, rhs_r.second);
                }
                
                bool BC_mortars_Q = true;
                if(BC_mortars_Q){ // apply BC on mortar
                    if(f.m_bc_type.first == 1 || f.m_bc_type.second == 1){
                        if (cell_ind == 0){
                            ...
                            size_t node_ind_l = f.m_bl_index;
                            size_t node_ind_r = f.m_br_index;
                            
                            auto hybrid_matrix_bc = skin_hybrid_matrix_at_bc_node(msh, face_l, face_r, f, cell_ind, cell_ind, node_ind_l, node_ind_r);
                            
                            std::cout << hybrid_matrix_bc << std::endl;
                            scatter_skin_hybrid_bc_data(msh, f_ind, f, cell_ind, bc_ind, hybrid_matrix_bc);
                            
                            scatter_skins_bc_u_t_data(msh, f_ind, f , bc_ind, 0);
//                            scatter_skins_bc_u_t_data(msh, f_ind, f , 1);
            ...
            // hybrid skin part
            size_t node_ind = 0;
            for (auto pair : f.m_internal_edges) {

                auto& chunk_m = f.m_pairs[node_ind];
                auto& face_m_l = storage->edges[chunk_m.first];
                auto& face_m_r = storage->edges[chunk_m.second];
                
                auto& chunk_p = f.m_pairs[node_ind+1];
                auto& face_p_l = storage->edges[chunk_p.first];
                auto& face_p_r = storage->edges[chunk_p.second];
                
                auto hybrid_matrix_l =  skin_hybrid_matrix_at_node(msh,face_m_l, face_p_l, f, node_ind, pair.first, true);
                auto hybrid_matrix_r =  skin_hybrid_matrix_at_node(msh,face_m_r, face_p_r, f, node_ind, pair.second, false);
                scatter_skin_hybrid_l_nodewise_data(msh, f_ind, f, node_ind, hybrid_matrix_l);
                scatter_skin_hybrid_r_nodewise_data(msh, f_ind, f, node_ind, hybrid_matrix_r);
                node_ind++;
            }
        ...
        // linking skins by means of mortar displacements
        bool point_mortars_Q = true;
        if(point_mortars_Q){
            size_t r_ind = 0;
            for (auto r : m_restrictions) {
                
                auto f_ind_l = r.m_f_index.first;
                auto f_ind_r = r.m_f_index.second;
                auto f_l = m_fractures[f_ind_l];
                auto f_r = m_fractures[f_ind_r];
                size_t n_cells_l = f_l.m_pairs.size();
                size_t n_cells_r = f_r.m_pairs.size();
                
                size_t node_l, face_ind_l, node_ind_l;
                bool side_Q_l;
                if (r.m_p_index.first == 0) {
                    if(r.m_s_index.first == 0){
                        face_ind_l = f_l.m_pairs[0].first;
                        side_Q_l = true;
                        node_ind_l = f_l.m_bl_index;
                    }else{
                        face_ind_l = f_l.m_pairs[0].second;
                        side_Q_l = false;
                        node_ind_l = f_l.m_br_index;
                    }
                    node_l = 0;
                    
                }else{
                    if(r.m_s_index.first == 0){
                        face_ind_l = f_l.m_pairs[n_cells_l-1].first;
                        side_Q_l = true;
                        node_ind_l = f_l.m_el_index;
                    }else{
                        face_ind_l = f_l.m_pairs[n_cells_l-1].second;
                        side_Q_l = false;
                        node_ind_l = f_l.m_er_index;
                    }
                    node_l = n_cells_l-1;
                }
                
                size_t node_r, face_ind_r, node_ind_r;
                bool side_Q_r;
                if (r.m_p_index.second == 0) {
                    if(r.m_s_index.second == 0){
                        face_ind_r = f_r.m_pairs[0].first;
                        side_Q_r = true;
                        node_ind_r = f_r.m_bl_index;
                    }else{
                        face_ind_r = f_r.m_pairs[0].second;
                        side_Q_r = false;
                        node_ind_r = f_r.m_br_index;
                    }
                    node_r = 0;
                }else{
                    if(r.m_s_index.second == 0){
                        face_ind_r = f_r.m_pairs[n_cells_r-1].first;
                        side_Q_r = true;
                        node_ind_r = f_r.m_el_index;
                    }else{
                        face_ind_r = f_r.m_pairs[n_cells_r-1].second;
                        side_Q_r = false;
                        node_ind_r = f_r.m_er_index;
                    }
                    node_r = n_cells_r-1;
                }
                
                assert(node_ind_l == node_ind_r);
                auto& face_l = storage->edges[face_ind_l];
                auto& face_r = storage->edges[face_ind_r];
                
                size_t node_ind = node_ind_l;
                auto hybrid_matrix =  skin_hybrid_matrix_at_mortar_node(msh, face_l, f_l, node_l, side_Q_l, face_r, f_r, node_r, side_Q_r, node_ind);
//                std::cout << hybrid_matrix << std::endl;
//                if(r_ind == 0){
...
//                }
        ...
        // local laws for stress points
        bool point_springs_Q = true;
        if(point_springs_Q){
            size_t s_ind = 0;
            for (auto s : m_springs) {
                
                auto f_ind_l = s.m_f_index.first;
                auto f_ind_r = s.m_f_index.second;
                auto f_l = m_fractures[f_ind_l];
                auto f_r = m_fractures[f_ind_r];
                size_t n_cells_l = f_l.m_pairs.size();
                size_t n_cells_r = f_r.m_pairs.size();
                ...
                size_t node_l, face_ind_l, node_ind_l;
                bool side_Q_l;
                if (s.m_p_index.first == 0) {
                    if(s.m_s_index.first == 0){
                        face_ind_l = f_l.m_pairs[0].first;
                        side_Q_l = true;
                        node_ind_l = f_l.m_bl_index;
                    }else{
                ...
                size_t node_r, face_ind_r, node_ind_r;
                bool side_Q_r;
                if (s.m_p_index.second == 0) {
                    if(s.m_s_index.second == 0){
                        face_ind_r = f_r.m_pairs[0].first;
                        side_Q_r = true;
                        node_ind_r = f_r.m_bl_index;
                    }else{
...
//                const Mesh& msh, size_t f_ind_l, fracture<Mesh> & f_l, size_t & node_ind_l, bool side_Q_l, size_t f_ind_r, fracture<Mesh> & f_r, const size_t & node_ind_r, bool side_Q_r, size_t & restrict_node, const Matrix<T, Dynamic, Dynamic>& mat
        ...
        if(point_restrictions_Q){ // apply restrictions
            ...
            for (auto r : m_restrictions) {
                ...
                auto& face_f = storage->edges[face_ind_f];
                auto& cell_f = storage->surfaces[cell_ind_f];
                auto& face_s = storage->edges[face_ind_s];
                auto& cell_s = storage->surfaces[cell_ind_s];
                
                const auto n_f = disk::normal(msh, cell_f, face_f);
                const auto t_f = disk::tanget(msh, cell_f, face_f);
                
                const auto n_s = disk::normal(msh, cell_s, face_s);
                const auto t_s = disk::tanget(msh, cell_s, face_s);
    ...
    auto skin_weighted_mass_matrix(const Mesh& msh, const typename Mesh::face_type& face_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
    ...
    auto skin_mass_matrix(const Mesh& msh, const typename Mesh::face_type& face_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
    ...
    auto skin_rhs(const Mesh& msh, const typename Mesh::face_type& face_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
    ...
    auto skin_hybrid_matrix(const Mesh& msh, const typename Mesh::face_type& face_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
    ...
    auto skin_hybrid_matrix_at_node(const Mesh& msh, const typename Mesh::face_type& face_m, const typename Mesh::face_type& face_p, fracture<Mesh> & f, const size_t & node_loc, const size_t & node_ind, bool left_Q = true)
        {
            auto storage = msh.backend_storage();
            elastic_material_data<T> & material = m_material[0];
            T rho = material.rho();
            ...
            auto degree = m_hho_di.face_degree();
            auto sm_basis = disk::make_scalar_monomial_basis(msh, face_m, degree);
            auto sp_basis = disk::make_scalar_monomial_basis(msh, face_p, degree);
            if(left_Q){
                if(f.m_flips_l.at(node_loc)){
                    sm_basis.swap_nodes();
                }
                if(f.m_flips_l.at(node_loc+1)){
                    sp_basis.swap_nodes();
                }
            }else{
                if(f.m_flips_r.at(node_loc)){
                    sm_basis.swap_nodes();
                }
                if(f.m_flips_r.at(node_loc+1)){
                    sp_basis.swap_nodes();
                }
            }

                        
            Matrix<T, Dynamic, Dynamic> ret = Matrix<T, Dynamic, Dynamic>::Zero(3, 2);

            typename Mesh::point_type point = storage->points.at(node_ind);
            
            auto nodes_m = sm_basis.nodes();
            typename Mesh::point_type point_ml = nodes_m[0];
            typename Mesh::point_type point_mr = nodes_m[1];
            typename Mesh::point_type vm = point_mr - point_ml;
            auto nm = inplane_normal(msh,face_m,point,vm);
            {
                const auto s_phi = sm_basis.eval_flux_functions(point);
                const auto mu_f_phi = sm_basis.eval_lambda_functions(point);
                const auto w_mu_f_phi = disk::priv::inner_product(1.0, mu_f_phi);
                const auto s_opt = disk::priv::outer_product(s_phi, w_mu_f_phi);
                
                ret.block(0,0,3,1) += -1.0 * nm * s_opt;
            }
            
            auto nodes_p = sp_basis.nodes();
            typename Mesh::point_type point_pl = nodes_p[0];
            typename Mesh::point_type point_pr = nodes_p[1];
            typename Mesh::point_type vp = point_pr - point_pl;
            auto np = inplane_normal(msh,face_p,point,vp);
            {
                const auto s_phi = sp_basis.eval_flux_functions(point);
                const auto mu_f_phi = sp_basis.eval_lambda_functions(point);
                const auto w_mu_f_phi = disk::priv::inner_product(1.0, mu_f_phi);
                const auto s_opt = disk::priv::outer_product(s_phi, w_mu_f_phi);
                
                ret.block(0,1,3,1) += -1.0 * np * s_opt;
            }
        
            return ret;
        }
    
    auto skin_hybrid_matrix_at_mortar_node(const Mesh& msh, const typename Mesh::face_type& face_l, fracture<Mesh> & f_l, const size_t & node_l, bool side_Q_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f_r, const size_t & node_r, bool side_Q_r, const size_t & node_ind)
        {
            auto storage = msh.backend_storage();
            elastic_material_data<T> & material = m_material[0];
            T rho = material.rho();
            ...
            auto sr_basis = disk::make_scalar_monomial_basis(msh, face_r, degree);
            if(side_Q_l){
                if(f_l.m_flips_l.at(node_l)){
                    sl_basis.swap_nodes();
                }
            }else{
                if(f_l.m_flips_r.at(node_l)){
                    sl_basis.swap_nodes();
                }
            }

            if(side_Q_r){
                if(f_r.m_flips_l.at(node_r)){
                    sr_basis.swap_nodes();
                }
            }else{
                if(f_r.m_flips_r.at(node_r)){
                    sr_basis.swap_nodes();
                }
            }
            ...
            typename Mesh::point_type point_lr = nodes_l[1];
            typename Mesh::point_type vl = point_lr - point_ll;
            auto nl = inplane_normal(msh,face_l,point,vl);
            {
                const auto s_phi = sl_basis.eval_flux_functions(point);
                const auto mu_f_phi = sl_basis.eval_lambda_functions(point);
                const auto w_mu_f_phi = disk::priv::inner_product(1.0, mu_f_phi);
                const auto s_opt = disk::priv::outer_product(s_phi, w_mu_f_phi);
                
                ret.block(0,0,3,1) += -1.0 * nl * s_opt;
            }
            ...
            auto nr = inplane_normal(msh,face_r,point,vr);
            {
                ...
                ret.block(0,1,3,1) += +1.0 * nr * s_opt;
            }
        
            return ret;
        }
    
    auto skin_hybrid_matrix_at_bc_node(const Mesh& msh, const typename Mesh::face_type& face_l, const typename Mesh::face_type& face_r, fracture<Mesh> & f, const size_t & cell_loc_l, const size_t & cell_loc_r, const size_t & node_ind_l, const size_t & node_ind_r)
        {
            auto storage = msh.backend_storage();
            elastic_material_data<T> & material = m_material[0];
            T rho = material.rho();
            ...
            auto sl_basis = disk::make_scalar_monomial_basis(msh, face_l, degree);
            auto sr_basis = disk::make_scalar_monomial_basis(msh, face_r, degree);
            if(f.m_flips_l.at(cell_loc_l)){
                sl_basis.swap_nodes();
            }
            if(f.m_flips_r.at(cell_loc_r)){
                sr_basis.swap_nodes();
            }
            ...
            typename Mesh::point_type point_l = storage->points.at(node_ind_l);
            typename Mesh::point_type point_r = storage->points.at(node_ind_r);
            
            auto nodes_l = sl_basis.nodes();
            typename Mesh::point_type point_ll = nodes_l[0];
            typename Mesh::point_type point_lr = nodes_l[1];
            typename Mesh::point_type vl = point_lr - point_ll;
            auto nl = inplane_normal(msh,face_l,point_l,vl);
            {
                const auto s_phi = sl_basis.eval_flux_functions(point_l);
                const auto mu_f_phi = sl_basis.eval_lambda_functions(point_l);
                const auto w_mu_f_phi = disk::priv::inner_product(1.0, mu_f_phi);
                const auto s_opt = disk::priv::outer_product(s_phi, w_mu_f_phi);
            ...
            typename Mesh::point_type point_rr = nodes_r[1];
            typename Mesh::point_type vr = point_rr - point_rl;
            auto nr = inplane_normal(msh,face_r,point_r,vr);
            {
                const auto s_phi = sr_basis.eval_flux_functions(point_r);
                const auto mu_f_phi = sr_basis.eval_lambda_functions(point_r);
                const auto w_mu_f_phi = disk::priv::inner_product(1.0, mu_f_phi);
                const auto s_opt = disk::priv::outer_product(s_phi, w_mu_f_phi);
    ...
    auto skin_coupling_matrix_ul(const Mesh& msh, const typename Mesh::cell_type& cell, const typename Mesh::face_type& face, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
            const auto degree     = m_hho_di.face_degree();
    ...
    auto skin_coupling_matrix_ur(const Mesh& msh, const typename Mesh::cell_type& cell, const typename Mesh::face_type& face, fracture<Mesh> & f, const size_t & cell_ind, size_t di = 0)
        {
            const auto degree     = m_hho_di.face_degree();
    ...
    auto skin_coupling_rhs_u(const Mesh& msh, const typename Mesh::cell_type& cell, const typename Mesh::face_type& face, fracture<Mesh> & f, const size_t & cell_ind, std::function<double(const typename Mesh::point_type& )> rhs_fun, size_t di = 0 )
        {
            const auto degree     = m_hho_di.face_degree();
            auto vec_u_basis = disk::make_vector_monomial_basis(msh, face, degree);
            
            Matrix<T, Dynamic, 1> ret_n = Matrix<T, Dynamic, 1>::Zero(vec_u_basis.size(),1);
            Matrix<T, Dynamic, 1> ret_t = Matrix<T, Dynamic, 1>::Zero(vec_u_basis.size(),1);
            ...
            for (auto& qp : qps)
            {
                ...
                const auto s_n_opt = rhs_fun(qp.point()) * w_n_dot_u_f_phi;
                const auto s_t_opt = rhs_fun(qp.point()) * w_t_dot_u_f_phi;
    ...
    void scatter_skin_weighted_rhs_u_data(const Mesh& msh, const size_t & face_id, size_t fracture_ind, fracture<Mesh> & f, const size_t & cell_ind, const Matrix<T, Dynamic, 1>& rhs)
    {
        size_t n_sigma_skin_bs = 3;
        ...
        auto face_LHS_offset = m_n_cells_dof + m_compress_indexes.at(face_id);
        for (size_t j = 0; j < n_fbs; j++)
        asm_map_i.push_back( assembly_index(face_LHS_offset+j, true));
...
//                std::cout << "ml = " << mass_matrix.first << std::endl;
//                std::cout << "rl = " << rhs.first << std::endl;
//                std::cout << "mr = " << mass_matrix.second << std::endl;
//                std::cout << "rr = " << rhs.second << std::endl;
...
//                std::cout << "xr = " << x_r << std::endl;
    ...
    size_t get_n_up_mortar_dof(){
        return m_n_up_mortar_dof;
    }
    
    size_t get_n_skin_cells_dof(){
        return m_n_skin_cells_dof;
    }
    
    size_t get_n_skin_edges_dof(){
        return m_n_skin_edges_dof;
    }
    ...
    std::vector<size_t> & compress_skin_cells_indexes(){
        return m_compress_skin_cells_indexes;
    }
    
    std::vector<size_t> & compress_skin_edges_indexes(){
        return m_compress_skin_edges_indexes;
    }
