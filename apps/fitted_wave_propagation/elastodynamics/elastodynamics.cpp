...
void IHHOSecondOrder(int argc, char **argv){
    ...
    std::ofstream simulation_log("elastic_one_field.txt");
    ...
    // Newmark process
    {
        ...
        for(size_t it = 1; it <= nt; it++){
            ...
            if(it == nt){
                postprocessor<mesh_type>::compute_errors_one_field_vectorial(msh, hho_di, assembler, u_dof_n, exact_vec_fun, exact_flux_fun);
            }
