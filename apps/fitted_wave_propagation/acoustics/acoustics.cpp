...
void EHHOFirstOrderCFL_II(int argc, char **argv);
...
void EHHOFirstOrderCFL(int argc, char **argv){
    ...
    int s = 1;
    int k_ind = sim_data.m_k_degree;
    std::vector<int> h_vec = {16,8,6,4};
    RealType mu_val = 2.0/((k_ind+1)*(k_ind+1));
    ...
    for(size_t l = 0; l <= sim_data.m_n_divs; l++){
        ...
        RealType lx = 1.0;
        RealType ly = 1.0;
        size_t nx = h_vec[k_ind]+l;
        size_t ny = h_vec[k_ind]+l;
        typedef disk::mesh<RealType, 2, disk::generic_mesh_storage<RealType, 2>>  mesh_type;
        typedef disk::BoundaryConditions<mesh_type, true> boundary_type;
        mesh_type msh;
